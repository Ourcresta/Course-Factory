...
// ============================================================
// ENTERPRISE SCHEMA - RBAC, APPROVALS, GOVERNANCE
// ============================================================
...
export const adminRolePermissionsRelations = relations(adminRolePermissions, ({ one }) => ({
  role: one(adminRoles, { fields: [adminRolePermissions.roleId], references: [adminRoles.id] }),
  permission: one(adminPermissions, { fields: [adminRolePermissions.permissionId], references: [adminPermissions.id] }),
}));
...
export const adminUserRolesRelations = relations(adminUserRoles, ({ one }) => ({
  user: one(users, { fields: [adminUserRoles.userId], references: [users.id] }),
...
export const approvalTemplates = pgTable("approval_templates", {
  ...
  description: text("description"),
  entityType: text("entity_type").notNull(),
  stepsConfig: jsonb("steps_config").$type<{
    order: number;
    roleRequired: string;
    isOptional: boolean;
...
export const insertApprovalTemplateSchema = createInsertSchema(approvalTemplates).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertApprovalTemplate = z.infer<typeof insertApprovalTemplateSchema>;
export type ApprovalTemplate = typeof approvalTemplates.$inferSelect;

// ==================== APPROVAL REQUESTS ====================
export const approvalRequests = pgTable("approval_requests", {
  id: serial("id").primaryKey(),
  templateId: integer("template_id").references(() => approvalTemplates.id),
  entityType: text("entity_type").notNull(),
...
export const approvalRequestsRelations = relations(approvalRequests, ({ one, many }) => ({
  template: one(approvalTemplates, { fields: [approvalRequests.templateId], references: [approvalTemplates.id] }),
...
export const approvalStepsRelations = relations(approvalSteps, ({ one }) => ({
  ...
  assignee: one(users, { fields: [approvalSteps.assignedTo], references: [users.id] }),
}));
...
export const approvalActionsRelations = relations(approvalActions, ({ one }) => ({
  request: one(approvalRequests, { fields: [approvalActions.requestId], references: [approvalRequests.id] }),
  step: one(approvalSteps, { fields: [approvalActions.stepId], references: [approvalSteps.id] }),
  actor: one(users, { fields: [approvalActions.actionBy], references: [users.id] }),
}));
...
export const courseVersionsRelations = relations(courseVersions, ({ one }) => ({
  course: one(courses, { fields: [courseVersions.courseId], references: [courses.id] }),
  creator: one(users, { fields: [courseVersions.createdBy], references: [users.id] }),
}));

export type CourseVersion = typeof courseVersions.$inferSelect;

// ==================== COURSE PUBLISH HISTORY ====================
export const coursePublishHistory = pgTable("course_publish_history", {
  id: serial("id").primaryKey(),
  courseId: integer("course_id").notNull().references(() => courses.id, { onDelete: "cascade" }),
  versionId: integer("version_id").references(() => courseVersions.id),
  action: text("action").notNull(),
...
export const coursePublishHistoryRelations = relations(coursePublishHistory, ({ one }) => ({
  course: one(courses, { fields: [coursePublishHistory.courseId], references: [courses.id] }),
  version: one(courseVersions, { fields: [coursePublishHistory.versionId], references: [courseVersions.id] }),
  performer: one(users, { fields: [coursePublishHistory.performedBy], references: [users.id] }),
}));

export type CoursePublishHistoryEntry = typeof coursePublishHistory.$inferSelect;

// ==================== COURSE QUALITY CHECKS ====================
export const courseQualityChecks = pgTable("course_quality_checks", {
  id: serial("id").primaryKey(),
  courseId: integer("course_id").notNull().references(() => courses.id, { onDelete: "cascade" }),
  versionId: integer("version_id").references(() => courseVersions.id),
...
export const courseQualityChecksRelations = relations(courseQualityChecks, ({ one }) => ({
  course: one(courses, { fields: [courseQualityChecks.courseId], references: [courses.id] }),
  version: one(courseVersions, { fields: [courseQualityChecks.versionId], references: [courseVersions.id] }),
}));

export type CourseQualityCheck = typeof courseQualityChecks.$inferSelect;
...
export const insertCreditPolicySchema = createInsertSchema(creditPolicies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const scholarshipPolicies = pgTable("scholarship_policies", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  description: text("description"),
...
export const insertScholarshipPolicySchema = createInsertSchema(scholarshipPolicies).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertScholarshipPolicy = z.infer<typeof insertScholarshipPolicySchema>;
export type ScholarshipPolicy = typeof scholarshipPolicies.$inferSelect;
...
// ==================== AI RULE REGISTRY ====================
export const aiRuleRegistry = pgTable("ai_rule_registry", {
  id: serial("id").primaryKey(),
  name: text("name").notNull().unique(),
  description: text("description"),
  ruleType: text("rule_type").notNull(),
...
export const insertAiRuleRegistrySchema = createInsertSchema(aiRuleRegistry).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertAiRuleRegistry = z.infer<typeof insertAiRuleRegistrySchema>;
export type AiRuleRegistry = typeof aiRuleRegistry.$inferSelect;
...
export const certificateTemplates = pgTable("certificate_templates", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  templateType: text("template_type").notNull().default("completion"),
  ...
  fieldsConfig: jsonb("fields_config").$type<{
    field: string;
...
// ==================== SIGNER REGISTRY ====================
export const signerRegistry = pgTable("signer_registry", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  title: text("title").notNull(),
...
export const insertSignerRegistrySchema = createInsertSchema(signerRegistry).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertSignerRegistry = z.infer<typeof insertSignerRegistrySchema>;
export type SignerRegistry = typeof signerRegistry.$inferSelect;

// ==================== ACADEMIC AUTHORITIES ====================
export const academicAuthorities = pgTable("academic_authorities", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  type: text("type").notNull(),
  ...
  accreditationDetails: jsonb("accreditation_details").$type<{
    type: string;
    issuedDate: string;
    expiryDate?: string;
    scope: string[];
...
export const insertAcademicAuthoritySchema = createInsertSchema(academicAuthorities).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertAcademicAuthority = z.infer<typeof insertAcademicAuthoritySchema>;
export type AcademicAuthority = typeof academicAuthorities.$inferSelect;
...
export const dataAccessLogs = pgTable("data_access_logs", {
  ...
  fieldsAccessed: jsonb("fields_accessed").$type<string[]>(),
  ipAddress: text("ip_address"),
  userAgent: text("user_agent"),
...
export const escalationLogsRelations = relations(escalationLogs, ({ one }) => ({
  assignee: one(users, { fields: [escalationLogs.escalatedTo], references: [users.id] }),
  resolver: one(users, { fields: [escalationLogs.resolvedBy], references: [users.id] }),
}));
...
export const adminSecurityLogsRelations = relations(adminSecurityLogs, ({ one }) => ({
  user: one(users, { fields: [adminSecurityLogs.userId], references: [users.id] }),
}));

export type AdminSecurityLog = typeof adminSecurityLogs.$inferSelect;
